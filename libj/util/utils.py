...
def save_re_result(i_th_res, args, fold, rel2idx):
    with open('./results/{0}/{1}/RE_RESULT_{2}.json'.format(args.task, args.bert_type, fold), 'w') as f:
        json.dump(i_th_res, f, indent=4, cls=NpEncoder)
    with open('./results/{0}/{1}/RE_rel2idx_{2}.json'.format(args.task, args.bert_type, fold), 'w') as f:
        json.dump(rel2idx, f, indent=4, cls=NpEncoder)


def save_ner_result(i_th_res, args, fold, tag2idx):
    with open('./results/{0}/{1}/NER_RESULT_{2}.json'.format(args.task, args.bert_type, fold), 'w') as f:
        json.dump(i_th_res, f, indent=4, cls=NpEncoder)
    with open('./results/{0}/{1}/NER_tag2idx_{2}.json'.format(args.task, args.bert_type, fold), 'w') as f:
