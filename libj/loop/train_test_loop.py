...
from libj.models import BERT_TF, compute_re_loss, compute_ner_loss, compute_loss
from libj.util import create_df, create_output_df, evaluate_rel, evaluate_ner, simple_evaluate_re, result2df, evaluate_rel_v2
...
def train_val_loop(train_vecs, ner_train_labels, re_train_gold_labels, 
    ...
    scheduler = transformers.get_linear_schedule_with_warmup(optimizer, 
                                                             ...
                                                             num_training_steps=len(train_vecs)*args.max_epoch)
    ...
    for epoch in range(args.max_epoch):
        ...
        for ofs in pbar_train:
            ...
            # 損失の計算
            re_loss = compute_re_loss(rel_logits, batch_re, device, weights.to(device))
        ...
        # 検証データの評価は簡易的に実施する
        ner_res = evaluate_ner(model, ner_val_labels, ner_preds, tag2idx)
        rel_res = simple_evaluate_re(re_val_gold_labels, re_preds, rel2idx)
        # 平均値
        val_F = 0.5 * ner_res["micro avg"]["f1-score"] + 0.5 * rel_res["micro avg"]["f1-score"]
        # 保存
        logger.info("検証")
...
def test_loop(X_test, test_vecs, ner_test_labels, re_test_gold_labels, 
    ...
    ner_res = evaluate_ner(model, ner_test_labels, ner_preds, tag2idx)
    return res_df, ner_res, rel_res