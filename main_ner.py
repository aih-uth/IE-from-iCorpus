...
import logging
from libp.util import load_data_cv, load_tokenizer, train_val_split_doc, make_idx, make_train_vecs, make_test_vecs, save_csv, save_ner_result
from libp.loop import train_val_loop_ner, test_loop_ner
import argparse
import json


def main():
    logger.info("----------{0}の実験を開始----------".format(args.bert_type))
    # 実験データ
    ...
    for fold in range(0, 5, 1):
        ...
        # テスト
        res_df, ner_res = test_loop_ner(X_test, test_vecs, ner_test_labels, fold, tag2idx, args, device)
        # 保存
        save_ner_result(ner_res, args, fold, tag2idx)
        save_csv(res_df, args, fold, "NER")
