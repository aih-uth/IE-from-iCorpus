...
def create_output_df(tmp_df, rel_preds_np, pred_tags, rel2idx):
    ...
    for i, preds in enumerate(rel_preds_np):
        for j, pred in enumerate(preds):
            ...
            if idx2rel[pred][0] == "R":
                # print("{0} - > {1}: {2}".format(i, j, pred_rel))
                rel_tail_list[i].append(str(j))
                rel_type_list[i].append(pred_rel[2:])
            else:
                # print("{0} - > {1}: {2}".format(j, i, pred_rel))
...
def save_re_result(i_th_res, args, fold, rel2idx):
    with open('./results/{0}/{1}/RE/RESULT_{2}.json'.format(args.task, args.bert_type, fold), 'w') as f:
        json.dump(i_th_res, f, indent=4, cls=NpEncoder)
    with open('./results/{0}/{1}/RE/rel2idx_{2}.json'.format(args.task, args.bert_type, fold), 'w') as f:
        json.dump(rel2idx, f, indent=4, cls=NpEncoder)


def save_ner_result(i_th_res, args, fold, tag2idx):
    with open('./results/{0}/{1}/NER/RESULT_{2}.json'.format(args.task, args.bert_type, fold), 'w') as f:
        json.dump(i_th_res, f, indent=4, cls=NpEncoder)
    with open('./results/{0}/{1}/NER/tag2idx_{2}.json'.format(args.task, args.bert_type, fold), 'w') as f:
        json.dump(tag2idx, f, indent=4, cls=NpEncoder)


def save_csv(fold_res_df, args, fold, name):
    fold_res_df.to_csv('./results/{0}/{1}/{3}/{2}.csv'.format(args.task, args.bert_type, fold, name), index=False)


# 交差エントロピー損失の重み
def get_weights(rel2idx, re_train_gold_labels):
    idx2rel = {v: k for k, v in rel2idx.items()}
    aggre = []
    for lines in re_train_gold_labels:
        lines = torch.triu(lines, diagonal=1)
        for line in lines:
            aggre.extend([idx2rel[x] for x in line.detach().cpu().numpy()])
    counts = collections.Counter(aggre)
    nSamples = []
    for k, _ in sorted(rel2idx.items(), key=lambda x: x[1]):
        if k == "PAD":
            nSamples.append(0)
        else:
            nSamples.append(counts[k])
    normedWeights = [1 - (x / sum(nSamples)) if x != rel2idx["PAD"] else 0 for x in nSamples]
